...
class Allocator:
    ...
    # 1) Brute-force = Optimal (Do as told. Linearly allocate and de-allocate):
    ...
    def allocate(self, size: int, m_id: int) -> int:  # TC = O(n); SC = O(1)
        consecutive_units = 0
        for i, unit in enumerate(self.arr):  # O(n+n) only because inner loop only executes single time
