...
def is_happy(n: int) -> bool:
    ...
    # For (understanding the) Complexity Analysis, read https://leetcode.com/problems/happy-number/editorial.
    ...
    while fast != 1 and slow != 1:  # TC = O(C)
        if slow == fast:
    ...
    # Why does these algos work? Because only two cases are possible here:
    # 1) `n` will reach to 1 => loop was not there, and `n` reached to 1
    # 2) `n` will NOT reach to 1 => loop was there, the reason why `n` didn't reach to 1,
    #                               and so our cycle finding algo will find it
    # 3) n will jump infinitely from one num to another? As numbers are infinite, it is possible right?
    #    NO! Why?:
    ...
    #    e.g. max(n) can be 2**31 - 1 = 2,147,483,647
    #    So, even if we take the biggest number which will have the next biggest number, it'd be 1,999,999,999.
    #    It's next = 730. So, that's what was being discussed in above quotation.
