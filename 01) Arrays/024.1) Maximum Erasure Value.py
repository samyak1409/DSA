...
def maximum_unique_subarray(nums: list[int]) -> int:
    ...
    # 0) Brute-force (Nested Loop & HashSet): TC = O(n^2); SC = O(n)

    # 1) Optimal (Sliding Window + HashMap + Prefix Sum): TC = O(n); SC = O(n)
    ...
    for index, num in enumerate(nums):
        prefix_sums.append(prefix_sum := prefix_sums[index]+num)
        if (last := last_index.get(num)) is not None:  # => num has occurred before also
            start = max(start, last+1)  # moving the start to index(num)+1 if start is not already ahead of it
